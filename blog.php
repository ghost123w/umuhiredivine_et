...
$stmt = $pdo->prepare("SELECT id FROM navigation_items WHERE label = 'BLOG' LIMIT 1");
$stmt->execute();
...
<head>
    ...
    <title>Blog | <?php echo htmlspecialchars(SITE_NAME); ?></title>
...
<body>
            ...
            <?php if (empty($fixtures)): ?>
                <div class="bento-card grid-large" style="text-align: center; display: flex; align-items: center; justify-content: center; min-height: 400px;">
                    <div class="card-content">
                        <h2 style="font-family: 'Cinzel', serif; color: var(--primary-color);">Stories are currently being written.</h2>
                        <p style="color: #666; font-size: 0.8rem; margin-top: 10px;">Check back soon for latest chronicles.</p>
                ...
                ?>
                    <div class="bento-card <?php echo $cardClass; ?>">
                        ...
                        <div class="card-content">
                            ...
                            <button class="card-btn" style="background: none; border: 1px solid var(--primary-color); color: #fff; padding: 10px 20px; border-radius: 4px; cursor: pointer; margin-top: 20px;" onclick="openContactModal('<?php echo addslashes($f['section_title']); ?>')">READ MORE</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <footer class="aura-footer">
        &copy; <?php echo date('Y'); ?> <?php echo htmlspecialchars(SITE_NAME); ?> &mdash; LUXURY CHRONICLES
    </footer>
