...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['update_settings'])) {
        ...
        if (isset($_FILES['section3_image_1']) && $_FILES['section3_image_1']['error'] == 0) {
            $target_dir = "../images/";
            if (!is_dir($target_dir)) mkdir($target_dir, 0755, true);

            $file_ext = strtolower(pathinfo($_FILES["section3_image_1"]["name"], PATHINFO_EXTENSION));
            $allowed_exts = ['jpg', 'jpeg', 'png', 'gif', 'webp'];

            if (in_array($file_ext, $allowed_exts)) {
                $new_filename = 'sec3_img1_' . uniqid() . '.' . $file_ext;
                $target_file = $target_dir . $new_filename;
                if (move_uploaded_file($_FILES["section3_image_1"]["tmp_name"], $target_file)) {
                    $image_path = 'images/' . $new_filename;
                    $stmt = $pdo->prepare("INSERT OR REPLACE INTO settings (setting_key, setting_value) VALUES ('section3_image_1', ?)");
        ...
        if (isset($_FILES['section3_image_2']) && $_FILES['section3_image_2']['error'] == 0) {
            $target_dir = "../images/";
            if (!is_dir($target_dir)) mkdir($target_dir, 0755, true);

            $file_ext = strtolower(pathinfo($_FILES["section3_image_2"]["name"], PATHINFO_EXTENSION));
            $allowed_exts = ['jpg', 'jpeg', 'png', 'gif', 'webp'];

            if (in_array($file_ext, $allowed_exts)) {
                $new_filename = 'sec3_img2_' . uniqid() . '.' . $file_ext;
                $target_file = $target_dir . $new_filename;
                if (move_uploaded_file($_FILES["section3_image_2"]["tmp_name"], $target_file)) {
                    $image_path = 'images/' . $new_filename;
                    $stmt = $pdo->prepare("INSERT OR REPLACE INTO settings (setting_key, setting_value) VALUES ('section3_image_2', ?)");
                    $stmt->execute([$image_path]);
                }
            }
        }
    ...
    if (isset($_POST['add_footer_item'])) {
        ...
        $stmt = $pdo->prepare("INSERT INTO footer_items (label, link_url, sort_order) VALUES (?, ?, ?)");
        $stmt->execute([$label, $url, $order]);
        header("Location: dashboard.php?view=footer&msg=added");
        exit();
    }

    if (isset($_POST['update_footer_item'])) {
        $id = (int)$_POST['id'];
        $label = sanitize($_POST['label']);
        $url = sanitize($_POST['link_url']);
        $order = (int)$_POST['sort_order'];
        ...
        $stmt = $pdo->prepare("UPDATE footer_items SET label = ?, link_url = ?, sort_order = ?, is_active = ? WHERE id = ?");
        $stmt->execute([$label, $url, $order, $active, $id]);
        header("Location: dashboard.php?view=footer&msg=updated");
        exit();
    }

    if (isset($_POST['delete_footer_item'])) {
        $id = (int)$_POST['id'];
        $stmt = $pdo->prepare("DELETE FROM footer_items WHERE id = ?");
        $stmt->execute([$id]);
        header("Location: dashboard.php?view=footer&msg=deleted");
        exit();
    }
}
...
$stmt->execute();
$section3_image_1 = $stmt->fetchColumn() ?: 'images/making.jpg';
            ...
            <?php if ($view == 'settings'): ?>
                <section class="aura-card">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <h3 style="font-family: 'Cinzel', serif; margin: 30px 0 20px; font-size: 1rem; color: var(--primary-color);">Section 3 (About Us) <span style="color: #fff;">Image Sides</span></h3>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
                            <div class="form-group">
                                <label style="color: #666; text-transform: uppercase; font-size: 0.6rem; letter-spacing: 2px;">Left Image Panel</label>
                                <input type="file" name="section3_image_1" class="form-control" style="background: rgba(255,255,255,0.03); color: #fff; border-color: rgba(255,255,255,0.05); padding: 15px;" accept="image/*">
                                <?php if ($section3_image_1): ?>
                                    <div style="margin-top: 10px; border-radius: 10px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1); width: 100px; height: 100px;">
                                        <img src="../<?php echo htmlspecialchars($section3_image_1); ?>" style="width: 100%; height: 100%; object-fit: cover;">
                                    </div>
                                <?php endif; ?>
                            </div>
                            <div class="form-group">
                                <label style="color: #666; text-transform: uppercase; font-size: 0.6rem; letter-spacing: 2px;">Right Image Panel</label>
                                <input type="file" name="section3_image_2" class="form-control" style="background: rgba(255,255,255,0.03); color: #fff; border-color: rgba(255,255,255,0.05); padding: 15px;" accept="image/*">
                                <?php if ($section3_image_2): ?>
                                    <div style="margin-top: 10px; border-radius: 10px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1); width: 100px; height: 100px;">
                                        <img src="../<?php echo htmlspecialchars($section3_image_2); ?>" style="width: 100%; height: 100%; object-fit: cover;">
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
